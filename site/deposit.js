function Deposit() {
    ...
    const updateUser = (theUser, action = undefined, amount = undefined) => {
        ...
        switch (action) {
            ...
            //     break;
            case 'deposit':
                let newBalance = theUser.balance + parseFloat(amount);
    ...
    function handleDeposit() {
        if(!validate(amount, 'checkEmpty', 'Please enter a deposit amount')) return;
        if(!validate(amount, 'checkValid', 'Please enter a valid deposit amount')) return;
        let userUpdated = updateUser(getLoggedInUser(), 'deposit', amount);
        if (userUpdated) {

            setBalance(formatter.format(getLoggedInUser().balance.toString()));
            setStatus('You have successfully made a ' + formatter.format(amount) + ' deposit.');
            setShow(false);
        }
    }
